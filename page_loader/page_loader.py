...
from page_loader.content import save_content, parse
...
def download(url, path):
    ...
    tags, soup = parse(url)
    data_to_load = prepare_page(url, tags, dir_for_resource)
    save_content(file_path, soup.prettify())
    download_resource(os.path.join(path, dir_for_resource),
                      data_to_load)
